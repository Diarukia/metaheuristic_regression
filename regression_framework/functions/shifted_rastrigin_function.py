...
class Shifted_rastrigin_function(Base_function):
    def __init__(self,name = 'Shifted_rastrigin_function',lower_bound = -100,upper_bound = 100,dimension = 100):
        super().__init__(name,lower_bound,upper_bound,dimension)
        self.bench = Benchmark()
        self.info = self.bench.get_info(2)
        self.dimension = self.info['dimension']
        self.calculus = self.bench.get_function(2)
        self.counter = 0
