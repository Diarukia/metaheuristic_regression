...
class One_separable_shifted_and_rotated_rastrigin_function(Base_function):
    def __init__(self,name = 'One_separable_shifted_and_rotated_rastrigin_function',lower_bound = -100,upper_bound = 100,dimension = 100):
        super().__init__(name,lower_bound,upper_bound,dimension)
        self.bench = Benchmark()
        self.info = self.bench.get_info(5)
        self.dimension = self.info['dimension']
        self.calculus = self.bench.get_function(5)
        self.counter = 0
